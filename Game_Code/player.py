...
class Player:
    ...
    def move(self,key):
        ...
        if (any([self.level_1,self.level_2,self.level_3]) and not (self.tutorial_one or self.tutorial_two) and not Win.condition(self) and not key[pygame.K_e] and not self.dialogue_condition and not self.dialogue_story_condition
    ...
    def attack(self,key):
        ...
        if any([self.level_1,self.level_2,self.level_3]) and not (self.tutorial_one or self.tutorial_two) and key[pygame.K_e]  and not Win.condition(self)  and self.player_attack_cooldown[0]>0 and not self.dialogue_condition and not self.dialogue_story_condition and not self.player_health[0]<=0 :
            self.player_x_movement[0]=0 ; self.player_y_movement[0]=0
