...
class GeneralBoss:
    ...
    def idle(self):
        ...
        if self.level_3 and self.distance>300:
    ...
    def move(self):
        ...
        self.distance=GeneralBoss.distance(self)
        if self.level_3 and self.distance<=300 and self.distance>100:
            BossGeneralFunctions.move(self,self.general_boss_rect,self.general_boss_move_flip,self.general_boss_move,self.general_boss_move_number,170,100,9,0.50,
    ...
    def attack(self):
        ...
        if self.general_boss_attack_type[0]==0:
            ...
            self.genreal_boss_attack=self.general_boss_attack_4 ; self.general_boss_attack_flip=self.general_boss_attack_4_flip ; self.player_health_reduction=150 ; self.max_attack_number=19

        if self.general_boss_attack_number[0]>self.max_attack_number:
            self.general_boss_attack_type[0]+=1
            if self.general_boss_attack_type[0]>3: self.general_boss_attack_type[0]=0

        if self.level_3 and self.distance<=100:
            BossGeneralFunctions.attack(self,self.general_boss_rect,self.general_boss_attack_flip,self.genreal_boss_attack,self.general_boss_attack_number,170,100,self.max_attack_number,0.50,self.general_boss_x_movement,
